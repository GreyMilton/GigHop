  ...
  return (
      <Pressable style={mainScreenStyles.EventSummaryCardPressableButton} onPress={() => navigation.navigate('EventScreen', { eventId: event['_id'], eventName: event['event_name']})}>
        <View>
          ...
          <Text style={mainScreenStyles.EventSummaryCardPressableText}>{(event['entry_price']["$numberDecimal"] === 0 || event['entry_price']["$numberDecimal"] === undefined || event['entry_price']["$numberDecimal"] === "0") ? 'Free entry' : "£" + event['entry_price']["$numberDecimal"] }</Text>
