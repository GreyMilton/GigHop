...
export default function MainScreen({ navigation }) {
  const venueArr = [
    {
      ...
      picture:
        "https://upload.wikimedia.org/wikipedia/commons/e/ef/The_Wiggles_live_in_Sydney_2018.jpg",
      ...
      venue_info: [
        {
          ...
          picture:
            "https://i2-prod.plymouthherald.co.uk/incoming/article579474.ece/ALTERNATES/s615/0_929382JPG.jpg",
          upcoming_events: [
            {
  ...
  const [venuesInCurrentViewWithGigs, setVenuesInCurrentViewWithGigs] =
    useState([]);
  const [mapMarkers, setMapMarkers] = useState([]);
  const [venueReferenceObject, setVenueReferenceObject] = useState({
  ...
  useEffect(() => {
    getEventsByTimestamp(selectedTimestamp).then((response) => {
      setFetchedEvents(response), setIsLoading(false);
    });
  }, [selectedTimestamp]);
  ...
  useEffect(() => {
    ...
    setVenueReferenceObject(
      createNewVenueReferenceObject(venuesInCurrentViewWithGigs)
    );
  ...
  return (
    <View style={mainScreenStyles.mainScreenContainer}>
      ...
      <View style={mainScreenStyles.mapViewSwitchContainer}>
        <Pressable
          style={mainScreenStyles.mapViewSwitchButton}
          onPress={switchDisplay}
        >
          <Text style={mainScreenStyles.mapViewSwitchTextSelected}>{`${
            mapIsDisplaying ? "Map" : "List"
          }`}</Text>
          <Text style={mainScreenStyles.mapViewSwitchTextNotSelected}>{`${
            mapIsDisplaying ? "/list" : "/map"
          }`}</Text>
        </Pressable>
      </View>
      {mapIsDisplaying ? (
        <MapDisplay
          ...
          venueReferenceObject={venueReferenceObject}
          isLoading={isLoading}
        />
      ) : (
        <EventListDisplay
